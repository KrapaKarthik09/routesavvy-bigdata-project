...
from pyspark.sql.functions import from_json, to_json, struct, col, year, month, dayofmonth, hour, when, lower
...
parsed = (parsed
    ...
    .withColumn("transfer_ratio", col("transfers") / (col("ridership") + 1))
    .select(
        "station_complex_id",
        "station_complex",
        "borough",
